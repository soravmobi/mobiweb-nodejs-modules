...
User.forgotPassword = function(req, res) {
    ...
    if (errors) {
        ...
        async.waterfall([
            ...
            forgotPasswordMsg += '<strong>Note: </strong>This temporary code will be valid for next '+constant.code_valid_time+' minutes. <br/><br/>';
            forgotPasswordMsg += 'Thanks <br/> '+siteName+' Team';

            let mailData = {};
            mailData.to_email = userEmail;
            mailData.subject  = '['+ siteName + '] forgot password';
            ...
            custom.sendEmailCallBack(mailData,function(err,resp){
                if(err){
                    ...
                    res.send({
                        ...
                        "response": {userLoginSessionKey:userLoginSessionKey},
                        "status": 5,
                        "message": "An email has been sent, please check your registered mail inbox."
                    });
                    return false; 
                }
            });

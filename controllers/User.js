            ...
            function(emailResults,isSocialEmail, callback) {
                ...
                database.pool.getConnection(function(err, connection) {
                    ...
                    connection.beginTransaction(function(err) {
                        ...
                        console.log('userTempCode',userTempCode);
                        ...
                        userDetailsObj.userRegistrationDate = constant.current_time;
                        ...
                        if(parseInt(isSocialEmail) === 0) // If email not get while social login (manual email)
                        {
                            ...
                            userDetailsObj.userTempCodeSentTime = constant.current_time;
                        }else{
                            userDetailsObj.userEmailVerified = 1;
                            userDetailsObj.noOfVerifiedSocialAccounts = 1;
                            userDetailsObj.userLastLogin = constant.current_time;
                        ...
                        connection.query(user_device_select_query, function(err, deviceResult) {
                            if (err) {
                                ...
                                deviceData.deviceModifiedDate = constant.current_time;
                                if (deviceResult != '') {
                                    ...
                                    deviceData.deviceAddedDate = constant.current_time;
    ...
    if (errors) {
        ...
        async.waterfall([
            function(callback) {
                ...
                /* Get users data */
                model.getSingleJoinData(function(err,userDetails){
                    if(err){
                        res.send(custom.dbErrorResponse());
                        return;
                    }else{
                ...
                },constant.users,constant.user_details,'masterUserId','userId','inner',{'user_details.userLoginSessionKey':userLoginSessionKey});
            },
            function(results, callback) {
                    ...
                    if(userSocialAccountDetails == ""){
                        ...
                        if(parseInt(userSocialAccountDetails[0].IsVerified === 1)){
                            ...
                            database.pool.getConnection(function(err, connection) {
                                ...
                                userUpdateData.userLastLogin = constant.current_time;
    ...
    if (errors) {
        ...
        async.waterfall([
            function(callback) {
                ...
                /* Get user details */
                model.getSingleJoinData(function(err,resp){
                    if(err){
                        res.send(custom.dbErrorResponse());
                        return false;
                    }else{
